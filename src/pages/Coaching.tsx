...
const programs = [
  { icon: '⭐', h3: 'Foundation Course', accent: 'coral', desc: 'For beginners. Build strong fundamentals across all four skills with comprehensive study materials.', points: ['60 hours of structured learning', 'Grammar & vocabulary building', 'Basic test strategies', 'Weekly progress assessments'] },
  { icon: '⚡', h3: 'Intensive Coaching', accent: 'blue', desc: 'Fast-track for quick score improvement. Intensive practice with expert one-on-one feedback.', points: ['40 hours of intensive training', 'Advanced test techniques', 'Mock tests & full analysis', 'Personal mentor support'] },
  { icon: '👤', h3: 'One-on-One Coaching', accent: 'green', desc: 'Fully personalised coaching tailored to your specific needs and learning pace.', points: ['Customised study plan', 'Flexible scheduling', 'Individual attention', 'Targeted skill improvement'] },
  { icon: '💻', h3: 'Online Masterclass', accent: 'dark', desc: 'Learn from anywhere with comprehensive online sessions and digital resources.', points: ['Live online sessions', 'Digital study materials', '24/7 platform access', 'Interactive practice tools'] },
];
...
export default function Coaching() {
  return (
    <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <div key={label} className="rounded-xl overflow-hidden shadow-md">
                ...
                <div className="border border-bdr border-t-0 p-6">
      ...
      <section className="py-20 bg-cream">
        <div className="max-w-content mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {programs.map(({ icon, h3, accent, desc, points }) => (
              <div key={h3} className={`bg-white border border-bdr border-t-4 ${accentBorderMap[accent]} rounded-xl p-7 shadow-sm hover:-translate-y-1 hover:shadow-lg transition-all duration-300`}>
                <h3 className="text-[17px] mb-2">{icon} {h3}</h3>
      ...
      <section className="py-20 bg-dark-section">
        <div className="max-w-content mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {extras.map(({ icon, h, body }) => (
              <div key={h} className="bg-white/[0.07] border border-white/10 rounded-xl p-5 text-center hover:bg-white/[0.12] transition-colors">
      ...
      {/* Booking form — cream */}
      <section id="coaching-form" className="py-20 bg-cream">
        <div className="max-w-content mx-auto px-6">
